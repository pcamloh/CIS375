...
namespace ACFramework
{
    ...
    class cCritterPlatform : cCritterWall
    {
        ...
        public cCritterPlatform(cVector3 penda, cVector3 pendb, Random prnd, int id, float pthickness = THICKNESS,
            float pheight = WALLPRISMDZ, cGame ppownergame = null)
            : base(penda, pendb, pthickness, pheight, ppownergame)
        {
            rnd = prnd;
            state = true;
            ...
            tmr.Interval = 100;
            tmr.Start();
            tmr.Elapsed += new System.Timers.ElapsedEventHandler(tmr_Tick);
            interval = rnd.Next(30, 61);
            elapsed = 0;
            enda = penda;
            endb = pendb;
            height = pheight;
        ...
        //constructor to appear/vanish at a fixed interval
        public cCritterPlatform(cVector3 penda, cVector3 pendb, int pinterval, float pthickness = THICKNESS,
            float pheight = WALLPRISMDZ, cGame ppownergame = null)
            : base(penda, pendb, pthickness, pheight, ppownergame)
        {
            x = pinterval;
        ...
        //TODO: constructor to float back and forth
        public cCritterPlatform(cVector3 penda, cVector3 pendb, cVector3 pnewenda, cVector3 pnewendb, int pinterval, float pthickness = THICKNESS,
            float pheight = WALLPRISMDZ, cGame ppownergame = null)
            : base(penda, pendb, pthickness, pheight, ppownergame)
        {
            //pinterval is the percent of the way there you get per tick.  .2 / pinterval is how many seconds it will take to complete a lap
            x = pinterval;
            state = true;
            tmr = new System.Timers.Timer();
            tmr.Interval = 33;
        ...
        */
        ...
        private void tmr_MoveTick(object sender, EventArgs e)
        {
            if (state)
            {
                try
        ...
        private void SwitchState()
        {
            if (state)
            {
                mySprite = this.Sprite;
                initialize(new cVector3(0, 0, 0), new cVector3(0, 0, 0), 0.0f, 0.0f, pownergame);
                state = false;
            }
            else
            {
