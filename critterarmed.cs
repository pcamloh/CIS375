...
{ 
    ...
    class cCritterBulletHyper : cCritterBulletSilver
    {
        public int knockBackRatio;
        public cCritterBulletHyper(int ratio) : base()
        {
            knockBackRatio = ratio;
        }

        //on collision, knock target 
        public override bool collide(cCritter pcritter)
        {
            bool success = base.collide(pcritter);
            if (success && pcritter.IsKindOf("cCritter3DPlayer"))
            {
                pcritter.addVelocity(new cVector3(this.Velocity.X * knockBackRatio, this.Velocity.Y * knockBackRatio, this.Velocity.Z * knockBackRatio));
            }
            return success;
        }
